...
contract TimelessNFT is ERC721Enumerable, Ownable {
    ...
    function payToMint(
        ...
        uint256 royality = (msg.value * royalityFee) / 100; //3 dua nhỏ được 3 đồng
        payTo(artist, royality);
        payTo(owner(), (msg.value - royality)); //97 chủ gia dịch
        //*****************************/
        //payTo(artist, (msg.value - royality));
        //payTo(owner(), royality);//97 chủ gia dịch
        //
        supply++;
    ...
    function payToBuy(uint256 id) external payable {
        ...
        payTo(artist, royality); // 3 dong vẫn là đứa nhỏ
        payTo(minted[id - 1].owner, (msg.value - royality)); // 97 đồng cho chủ sở hữu bán được
